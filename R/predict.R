...
##' Prediction with new data and a saved forest from CrazyForest.
##'
...
##' To retrieve the corresponding factor levels, use \code{rf$forest$levels}, if \code{rf} is the crazyforest object.
##'
...
##'
##' @title CrazyForest prediction
##' @param object CrazyForest \code{crazyforest.forest} object.
...
##' @return Object of class \code{crazyforest.prediction} with elements
##'   \tabular{ll}{
...
predict.crazyforest.forest <- function(object, data, predict.all = FALSE,
  ...
  if (!inherits(object, "crazyforest.forest")) {
  ...
  ## Check for old crazyforest version
  if (length(forest$child.nodeIDs) != forest$num.trees || length(forest$child.nodeIDs[[1]]) < 2 || length(forest$child.nodeIDs[[1]]) > 3) {
    stop("Error: Invalid forest object. Is the forest grown in crazyforest version <0.3.9? Try to predict with the same version the forest was grown.")
  }
  if (!is.null(forest$dependent.varID)) {
    warning("Forest grown in crazyforest version <0.11.5, converting ...")
  ...
  if (type == "response" || type == "se") {
    ...
    stop("Error: Apply predict() to the crazyforest object instead of the $forest object to predict quantiles.")
  } else {
  ...
  if (type == "se" && is.null(inbag.counts)) {
    stop("Error: No saved inbag counts in crazyforest object. Please set keep.inbag=TRUE when calling crazyforest.")
  }
  ...
  if (!is.null(forest$predict_strategy)) {
    predict_strategy <- forest$predict_strategy
  } else {
    predict_strategy <- "majority"
  }
  if (!is.null(forest$class_frequencies)) {
    class_frequencies <- forest$class_frequencies
  }
  ...
  result <- crazyforestCpp(
    treetype, x, y, forest$independent.variable.names, mtry,
  ...
  class(result) <- "crazyforest.prediction"
  return(result)
}
...
##'
##' @title CrazyForest prediction
##' @param object CrazyForest \code{crazyforest} object.
##' @param data New test data of class \code{data.frame} or \code{gwaa.data} (GenABEL).
##' @param predict.all Return individual predictions for each tree instead of aggregated predictions for all trees. Return a matrix (sample x tree) for classification and regression, a 3d array for probability estimation (sample x class x tree) and survival (sample x time x tree).
##' @param num.trees Number of trees used for prediction. The first \code{num.trees} in the forest are used.
##' @param type Type of prediction. One of 'response', 'se', 'terminalNodes', 'quantiles' with default 'response'. See below for details.
...
predict.crazyforest <- function(object, data = NULL, predict.all = FALSE,
  ...
  if (type == "quantiles") {
    ...
    if (is.null(object$random.node.values)) {
      stop("Error: Set quantreg=TRUE in crazyforest(...) for quantile prediction.")
    }

    if (is.null(data)) {
      ## OOB prediction
      if (is.null(object$random.node.values.oob)) {
        stop("Error: Set keep.inbag=TRUE in crazyforest(...) for out-of-bag quantile prediction or provide new data in predict(...).")
      }
