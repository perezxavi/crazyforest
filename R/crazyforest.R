...
##' Crazyforest is a fast implementation of random forests (Breiman 2001) or recursive partitioning, particularly suited for high dimensional data.
...
##'
##' Note that crazyforest has different default values than other packages.
...
##' This can also be used to grow a single decision tree without bagging and feature subsetting: \code{crazyforest(..., num.trees = 1, mtry = p, replace = FALSE, sample.fraction = 1)}, where p is the number of independent variables.
##'
...
##' To find the best hyperparameters, consider hyperparameter tuning with the \code{tuneCrazyForest} or \code{mlr3} packages.
##'
...
##' With \code{na.action = "na.learn"}, in each node either all missings go left or all missings go right. The direction is chosen based on the split criterion value (i.e., decrease of impurity).
...
##'
##' For GWAS data consider combining \code{crazyforest} with the \code{GenABEL} package.
...
##'
##' By default, crazyforest uses 2 threads. The default can be changed with: (1) \code{num.threads} in crazyforest/predict call, (2) environment variable
##' R_CRAZYFOREST_NUM_THREADS, (3) \code{options(crazyforest.num.threads = N)}, (4) \code{options(Ncpus = N)}, with precedence in that order.
##'
##' See \url{https://github.com/imbs-hl/crazyforest} for the development version.
##'
##' @title Crazyforest
...
##' @param write.forest Save \code{crazyforest.forest} object, required for prediction. Set to \code{FALSE} to reduce memory usage if no prediction intended.
...
##' @return Object of class \code{crazyforest} with elements
...
##' rg.iris <- crazyforest(Species ~ ., data = iris.train)
##' pred.iris <- predict(rg.iris, data = iris.test)
##' table(iris.test$Species, pred.iris$predictions)
##'
##' ## Quantile regression forest
##' rf <- crazyforest(mpg ~ ., mtcars[1:26, ], quantreg = TRUE)
##' pred <- predict(rf, mtcars[27:32, ], type = "quantiles")
##' pred$predictions
##'
##' ## Variable importance
##' rg.iris <- crazyforest(Species ~ ., data = iris, importance = "impurity")
##' rg.iris$variable.importance
##'
##' ## Survival forest
##' require(survival)
##' rg.veteran <- crazyforest(Surv(time, status) ~ ., data = veteran)
##' plot(rg.veteran$unique.death.times, rg.veteran$survival[1,])
##'
##' ## Alternative interfaces (same results)
##' crazyforest(dependent.variable.name = "Species", data = iris)
##' crazyforest(y = iris[, 5], x = iris[, -5])
##'
##' \dontrun{
...
##' crazyforest(trait ~ ., data = dat.gwaa)
##' }
##'
##' @author Javier Pérez-Rodríguez
##' @references
##' \itemize{
##'   \item PÃ©rez-RodrÃ­guez, J. & GarcÃ­a-Pedrajas, N. (2017). crazyforest: A fast implementation of random forests for high dimensional data in C++ and R. J Stat Softw 77:1-17. \doi{10.18637/jss.v077.i01}.
##'   \item Schmid, M., PÃ©rez-RodrÃ­guez, J. & GarcÃ­a-Pedrajas, N. (2016). On the use of Harrell's C for clinical risk prediction via random survival forests. Expert Syst Appl 63:450-459. \doi{10.1016/j.eswa.2016.07.018}.
##'   \item PÃ©rez-RodrÃ­guez, J., Dankowski, T. & GarcÃ­a-Pedrajas, N. (2017). Unbiased split variable selection for random survival forests using maximally selected rank statistics. Stat Med 36:1272-1284. \doi{10.1002/sim.7212}.
##'   \item Nembrini, S., Koenig, I. R. & PÃ©rez-RodrÃ­guez, J. (2018). The revival of the Gini Importance? Bioinformatics. \doi{10.1093/bioinformatics/bty373}.
...
##'   \item Malley, J. D., Kruppa, J., Dasgupta, A., Malley, K. G., & GarcÃ­a-Pedrajas, N. (2012). Probability machines: consistent probability estimation using nonparametric learning machines. Methods Inf Med 51:74-81. \doi{10.3414/ME00-01-0052}.
...
##'   \item Probst, P., PÃ©rez-RodrÃ­guez, J. & Boulesteix, A-L. (2019). Hyperparameters and tuning strategies for random forest. WIREs Data Mining Knowl Discov 9:e1301.\doi{10.1002/widm.1301}.
##'   }
##' @seealso \code{\link{predict.crazyforest}}
##' @useDynLib crazyforest, .registration = TRUE
##' @importFrom Rcpp evalCpp
##' @import stats
##' @import utils
##' @importFrom Matrix Matrix
##' @export
crazyforest <- function(formula = NULL, data = NULL, num.trees = 500, mtry = NULL,
                        ...
                        poisson.tau = 1, predict_strategy = "majority",
  ...
  predict_strategy <- match.arg(predict_strategy, c("majority", "roulette", "corrected_roulette"))
  ...
  if (is.null(data)) {
    ...
    if (is.null(formula)) {
      ...
      if (ncol(data) > 10000) {
        warning("Avoid the formula interface for high-dimensional data. If crazyforest is slow or you get a 'protection stack overflow' error, consider the x/y or dependent.variable.name interface (see examples).")
      }
  ...
  if (is.null(num.threads)) {
    num.threads <- as.integer(Sys.getenv("R_CRAZYFOREST_NUM_THREADS", getOption("crazyforest.num.threads", getOption("Ncpus", 2L))))
  ...
  if (length(sample.fraction) > 1) {
    ...
    if (!replace & any(sample.fraction * length(y) > table(y))) {
      ...
      stop(
        "Error: Not enough samples in class ", names(idx),
  ...
  if (is.null(splitrule)) {
    ...
    if (treetype %in% c(1, 9)) {
      ...
      stop("Error: totally_random splitrule only implemented for classification.")
    }
  } else if (splitrule == "chaos_split") {
    if (treetype %in% c(1, 9)) {
      splitrule.num <- 10
    } else {
      stop("Error: chaos_split splitrule only implemented for classification.")
    }
  } else {
    ...
    if (length(unordered.factor.variables) > 0) {
      ...
      if (max(num.levels) > max.level.count) {
        stop(paste("Too many levels in unordered categorical variable ", unordered.factor.variables[which.max(num.levels)],
          ". Only ", max.level.count, " levels allowed on this system. Consider using the 'order' option.",
          sep = ""
        ))
      }
    } else {
  ...
  ## Prediction mode always false. Use predict.crazyforest() method.
  ...
  ## Call CrazyForest
  class_frequencies <- numeric(0)
  if (predict_strategy == "majority") {
    predict_strategy_int <- 1
  } else if (predict_strategy == "roulette") {
    ...
    if (treetype %in% c(1, 9)) {
      ...
      class_frequencies <- class_frequencies[unique(as.numeric(y))]
    } else {
  ...
  if (treetype == 1 && oob.error) {
    if (is.factor(y)) {
      result$predictions <- integer.to.factor(
        result$predictions,
        levels(y)
      )
    }
  ...
  if (write.forest) {
    ...
    result$forest$predict_strategy <- predict_strategy
    result$forest$class_frequencies <- class_frequencies
    class(result$forest) <- "crazyforest.forest"
  ...
  result$predict_strategy <- predict_strategy
  class(result) <- "crazyforest"
