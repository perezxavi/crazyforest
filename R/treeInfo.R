...
#'
#' Extract tree information of a \code{crazyforest} object.
#'
#' Node and variable ID's are 0-indexed, i.e., node 0 is the root node.
#' If the formula interface is used in the \code{crazyforest} call, the variable ID's are usually different to the original data used to grow the tree.
#' Refer to the variable name instead to be sure.
#'
#' Splitting at unordered factors (nominal variables) depends on the option \code{respect.unordered.factors} in the \code{crazyforest} call.
...
#'
#' @param object \code{crazyforest} object.
...
#'       \code{numSamples} \tab Number of samples in the node (only if crazyforest called with \code{node.stats = TRUE}). \cr
#'       \code{splitStat} \tab Split statistics, i.e., value of the splitting criterion (only if crazyforest called with \code{node.stats = TRUE}). \cr
#'   }
#' @examples
#' rf <- crazyforest(Species ~ ., data = iris)
#' treeInfo(rf, 1)
#' @seealso \code{\link{crazyforest}}
#' @author Javier Pérez-Rodríguez
#' @export
treeInfo <- function(object, tree = 1) {
  if (!inherits(object, "crazyforest")) {
  ...
  if (length(forest$child.nodeIDs) != forest$num.trees || length(forest$child.nodeIDs[[1]]) < 2 || length(forest$child.nodeIDs[[1]]) > 3) {
    stop("Error: Invalid forest object. Is the forest grown in crazyforest version <0.3.9? Try with the same version the forest was grown.")
  }
  ...
  result <- data.frame(
    nodeID = 0:(length(forest$split.values[[tree]]) - 1),
