library(crazyforest)
library(survival)
context("crazyforest_extratrees")

test_that("extratrees splitting works for classification", {
  rf <- crazyforest(Species ~ ., iris, splitrule = "extratrees")
...
test_that("extratrees unordered splitting works for classification", {
  ...
  rf <- crazyforest(y ~ ., data = dat, num.trees = 5, min.node.size = n/2, 
...
test_that("extratrees splitting works for probability estimation", {
  rf <- crazyforest(Species ~ ., iris, probability = TRUE, splitrule = "extratrees")
  expect_is(rf, "crazyforest")
  expect_lt(rf$prediction.error, 0.2)
})
...
test_that("extratrees splitting works for regression", {
  rf <- crazyforest(Sepal.Length ~ ., iris, splitrule = "extratrees")
  expect_is(rf, "crazyforest")
  expect_gt(rf$r.squared, 0.5)
})

test_that("extratrees unordered splitting works for regression", {
  rf <- crazyforest(Sepal.Length ~ ., iris, splitrule = "extratrees", respect.unordered.factors = "partition")
  expect_is(rf, "crazyforest")
  expect_gt(rf$r.squared, 0.5)
})

test_that("extratrees splitting works for survival", {
  rf <- crazyforest(Surv(time, status) ~ ., veteran, splitrule = "extratrees")
  expect_is(rf, "crazyforest")
  expect_lt(rf$prediction.error, 0.4)
})

test_that("extratrees unordered splitting works for survival", {
  rf <- crazyforest(Surv(time, status) ~ ., veteran, splitrule = "extratrees", respect.unordered.factors = "partition")
  expect_is(rf, "crazyforest")
  expect_lt(rf$prediction.error, 0.4)
})

test_that("extratrees splitting works for large number of random splits", {
  expect_silent(crazyforest(Species ~ ., iris, splitrule = "extratrees", num.random.splits = 100))
})
