...
test_that("Matrix interface works for Probability estimation", {
  rf <- crazyforest(dependent.variable.name = "Species", data = data.matrix(iris), write.forest = TRUE, probability = TRUE)
...
test_that("Matrix interface prediction works for Probability estimation", {
  ...
  rf <- crazyforest(dependent.variable.name = "Species", data = dat, write.forest = TRUE, probability = TRUE)
...
test_that("no warning if data.frame has two classes", {
  ...
  expect_silent(crazyforest(Species ~ ., data = dat))
})

test_that("Error if sample fraction is 0 or >1", {
  expect_error(crazyforest(Species ~ ., iris, num.trees = 5, sample.fraction = 0))
  expect_error(crazyforest(Species ~ ., iris, num.trees = 5, sample.fraction = 1.1))
})

test_that("Error if sample fraction is vector for regression", {
  expect_error(crazyforest(Sepal.Length ~ ., iris, num.trees = 5, sample.fraction = c(0.1, 0.2)), 
...
test_that("Error if sample fraction is vector of wrong size", {
  expect_error(crazyforest(Species ~ ., iris, num.trees = 5, sample.fraction = c(0.1, 0.2)), 
...
test_that("Error if element of sample fraction vector is <0 or >1", {
  expect_error(crazyforest(Species ~ ., iris, num.trees = 5, sample.fraction = c(0.1, 1.1, 0.3)), 
               "Error: Invalid value for sample\\.fraction. Please give a value in \\(0,1\\] or a vector of values in \\[0,1\\]\\.")
  expect_error(crazyforest(Species ~ ., iris, num.trees = 5, sample.fraction = c(-3, 0.5, 0.3)), 
               "Error: Invalid value for sample.fraction. Please give a value in \\(0,1] or a vector of values in \\[0,1\\]\\.")
})

test_that("Error if sum of sample fraction vector is 0", {
  expect_error(crazyforest(Species ~ ., iris, num.trees = 5, sample.fraction = c(0, 0, 0)), 
               "Error: Invalid value for sample\\.fraction. Sum of values must be >0\\.")
})

test_that("Error if replace=FALSE and not enough samples", {
  expect_error(crazyforest(Species ~ ., iris, num.trees = 5, sample.fraction = c(0.2, 0.3, 0.4), 
                      replace = FALSE, keep.inbag = TRUE), 
               "Error: Not enough samples in class virginica; available: 50, requested: 60.")
  expect_silent(crazyforest(Species ~ ., iris, num.trees = 5, sample.fraction = c(0.2, 0.3, 0.4), 
...
test_that("Inbag counts match sample fraction, classification", {
  ## With replacement
  rf <- crazyforest(Species ~ ., iris, num.trees = 5, sample.fraction = c(0.2, 0.3, 0.4), 
               replace = TRUE, keep.inbag = TRUE)
  ...
  ## Without replacement
  rf <- crazyforest(Species ~ ., iris, num.trees = 5, sample.fraction = c(0.1, 0.2, 0.3), 
               replace = FALSE, keep.inbag = TRUE)
  inbag <- do.call(cbind, rf$inbag.counts)
  ...
  rf <- crazyforest(Species ~ ., dat, num.trees = 5, sample.fraction = c(0.1, 0.2, 0.3), 
               replace = FALSE, keep.inbag = TRUE)
  inbag <- do.call(cbind, rf$inbag.counts)
  ...
  ## No factor outcome
  rf <- crazyforest(Species ~ ., data.matrix(iris), num.trees = 5, sample.fraction = c(0.2, 0.3, 0.4), 
...
test_that("as.factor() in formula works", {
  ...
  expect_silent(crazyforest(as.factor(y) ~ ., data = dt, num.trees = 5, write.forest = TRUE))
})

test_that("holdout mode holding out data with 0 weight", {
  weights <- rbinom(nrow(iris), 1, 0.5)
  rf <- crazyforest(Species ~ ., iris, num.trees = 5, importance = "permutation",  
...
test_that("holdout mode not working if no weights", {
  expect_error(crazyforest(Species ~ ., iris, num.trees = 5, importance = "permutation", holdout = TRUE))
})
...
test_that("OOB error is correct for 1 tree, classification", {
  ...
  rf <- crazyforest(y ~ ., dat, num.trees = 1)
...
test_that("OOB error is correct for 1 tree, probability prediction", {
  n <- 50
  dat <- data.frame(y = factor(rbinom(n, 1, .5)), x = rnorm(n))
  rf <- crazyforest(y ~ ., dat, num.trees = 1, probability = TRUE)
...
test_that("Split points are at (A+B)/2 for numeric features, regression variance splitting", {
  ...
  rf <- crazyforest(y ~ x, dat, num.trees = 10)
...
test_that("Split points are at (A+B)/2 for numeric features, regression maxstat splitting", {
  dat <- data.frame(y = rbinom(100, 1, .5), x = rbinom(100, 1, .5))
  rf <- crazyforest(y ~ x, dat, num.trees = 10, splitrule = "maxstat", alpha = 1)
  split_points <- sapply(1:rf$num.trees, function(i) {
    res <- treeInfo(rf, i)$splitval
    res[!is.na(res)]
  })
...
test_that("Split points are at (A+B)/2 for numeric features, probability", {
  dat <- data.frame(y = factor(rbinom(100, 1, .5)), x = rbinom(100, 1, .5))
  rf <- crazyforest(y ~ x, dat, num.trees = 10, probability = TRUE)
  split_points <- sapply(1:rf$num.trees, function(i) {
    res <- treeInfo(rf, i)$splitval
    res[!is.na(res)]
  })
...
test_that("Split points are at (A+B)/2 for numeric features, survival logrank splitting", {
  ...
  rf <- crazyforest(Surv(time, status) ~ x, dat, num.trees = 10, splitrule = "logrank")
  split_points <- sapply(1:rf$num.trees, function(i) {
    res <- treeInfo(rf, i)$splitval
    res[!is.na(res)]
  })
...
test_that("Split points are at (A+B)/2 for numeric features, survival C-index splitting", {
  dat <- data.frame(time = runif(100, 1, 10), status = rbinom(100, 1, .5), x = rbinom(100, 1, .5))
  rf <- crazyforest(Surv(time, status) ~ x, dat, num.trees = 10, splitrule = "C")
  split_points <- sapply(1:rf$num.trees, function(i) {
    res <- treeInfo(rf, i)$splitval
    res[!is.na(res)]
  })
...
test_that("Split points are at (A+B)/2 for numeric features, survival maxstat splitting", {
  dat <- data.frame(time = runif(100, 1, 10), status = rbinom(100, 1, .5), x = rbinom(100, 1, .5))
  rf <- crazyforest(Surv(time, status) ~ x, dat, num.trees = 10, splitrule = "maxstat", alpha = 1)
  split_points <- sapply(1:rf$num.trees, function(i) {
    res <- treeInfo(rf, i)$splitval
    res[!is.na(res)]
  })
...
test_that("No error if variable named forest", {
  ...
  rf <- crazyforest(Species ~ ., dat, num.trees = 5)
  expect_silent(predict(rf, dat))
})
...
test_that("Prediction error is NA if oob.error=FALSE", {
  rf <- crazyforest(Species ~ ., iris, num.trees = 5, oob.error = FALSE)
  expect_true(is.na(rf$prediction.error))
  
  rf <- crazyforest(Surv(time,status) ~ ., veteran, num.trees = 5, oob.error = FALSE)
  expect_true(is.na(rf$prediction.error))
})
...
test_that("Tree depth creates trees of correct size", {
  ...
  # Depth 1
  rf <- crazyforest(Species ~ ., iris, num.trees = 5, max.depth = 1)
  expect_true(all(forest_depth(rf) <= 1))
  
  # Depth 4
  rf <- crazyforest(Species ~ ., iris, num.trees = 5, max.depth = 4)
  ...
  rf <- crazyforest(y ~ ., dat, num.trees = 5, min.node.size = 1, max.depth = max.depth)
  expect_true(all(forest_depth(rf) <= max.depth))
})

test_that("Tree depth 0 equivalent to unlimited", {
  set.seed(200)
  rf1 <- crazyforest(Species ~ ., iris, num.trees = 5, max.depth = 0)
  
  set.seed(200)
  rf2 <- crazyforest(Species ~ ., iris, num.trees = 5)
...
test_that("Meaningful predictions with max.depth = 1", {
  rf <- crazyforest(Sepal.Length ~ ., iris, max.depth = 1, num.trees = 5)
...
test_that("Does not crash when variable named 'none'", {
  dat <- data.frame(y = rbinom(100, 1, .5), 
                    x = rbinom(100, 1, .5), 
                    none = rbinom(100, 1, .5))
  rf <- crazyforest(data = dat, dependent.variable.name = "y")
  expect_equal(rf$forest$independent.variable.names, c("x", "none"))
  expect_silent(predict(rf, dat))
})

test_that("mtry function input works as expected", {
  rf <- crazyforest(Species ~ ., data = iris, mtry = function(n) n - 1)
  expect_equal(3, rf$mtry)
})


test_that("mtry function error halts the crazyforest function", {
  expect_error(
    crazyforest(Species ~ ., data = iris, mtry = function(n) stop("this is some error")), 
...
test_that("min.bucket creates nodes of correct size", {
  ...
  # Size 2
  rf <- crazyforest(Species ~ ., iris, num.trees = 5, replace = FALSE, 
...
test_that("Vector min.bucket creates nodes of correct size", {
  ...
  # No factor outcome
  rf <- crazyforest(Species ~ ., data.matrix(iris), num.trees = 5, replace = FALSE, 
