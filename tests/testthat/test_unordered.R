...
test_that("Old parameters still work", {
  ...
  rf.false <- crazyforest(y ~ ., data = dt, num.trees = 5, write.forest = TRUE, 
                     respect.unordered.factors = FALSE)
  rf.true <- crazyforest(y ~ ., data = dt, num.trees = 5, write.forest = TRUE, 
...
test_that("If respect.unordered.factors='partition', regard characters as unordered", {
  ...
  set.seed(2)
  rf.char <- crazyforest(y ~ ., data = dt, num.trees = 5, min.node.size = n/2, respect.unordered.factors = 'partition')
  
  dt$x <- factor(dt$x, ordered = FALSE)
  set.seed(2)
  rf.fac <- crazyforest(y ~ ., data = dt, num.trees = 5, min.node.size = n/2, respect.unordered.factors = 'partition')
...
test_that("If respect.unordered.factors='ignore', regard characters as ordered", {
  ...
  set.seed(2)
  rf.char <- crazyforest(y ~ ., data = dt, num.trees = 5, min.node.size = n/2, respect.unordered.factors = 'ignore')
  
  dt$x <- factor(dt$x, ordered = FALSE)
  set.seed(2)
  rf.fac <- crazyforest(y ~ ., data = dt, num.trees = 5, min.node.size = n/2, respect.unordered.factors = 'ignore')
  
  expect_equal(rf.char$prediction.error, rf.fac$prediction.error)
})

test_that("Error if other value for respect.unordered.factors", {
  expect_error(crazyforest(y ~ ., iris, num.trees = 5, respect.unordered.factors = NULL))
})

test_that("Same results if no unordered factors", {
  set.seed(100)
  rf1 <- crazyforest(Species ~ ., iris, num.trees = 5, respect.unordered.factors = 'ignore')
  set.seed(100)
  rf2 <- crazyforest(Species ~ ., iris, num.trees = 5, respect.unordered.factors = 'order')
  set.seed(100)
  rf3 <- crazyforest(Species ~ ., iris, num.trees = 5, respect.unordered.factors = 'partition')
...
test_that("Unordered splitting working for classification", {
  ...
  rf <- crazyforest(y ~ ., data = dt, num.trees = 5, min.node.size = n/2, respect.unordered.factors = 'partition')
...
test_that("Unordered splitting working for probability", {
  ...
  rf <- crazyforest(y ~ ., data = dt, num.trees = 5, min.node.size = n/2, 
...
test_that("Order splitting working for multiclass classification", {
  ...
  rf <- crazyforest(y ~ ., data = dt, num.trees = 5, 
...
test_that("Order splitting working with alternative interface", {
  ...
  rf <- crazyforest(dependent.variable.name = "y", data = dt, num.trees = 5, respect.unordered.factors = 'order')
  expect_true(all(rf$forest$is.ordered))
  
  rf <- crazyforest(dependent.variable.name = "y", data = dt, num.trees = 5, respect.unordered.factors = 'order', probability = TRUE)
  expect_true(all(rf$forest$is.ordered))
})
...
test_that("Order splitting working with single level factor", {
  ...
  dt_class <- data.frame(x = sample(c("A"), n, replace = TRUE), 
                         ...
                         stringsAsFactors = FALSE)
  expect_silent(crazyforest(y ~ ., data = dt_class, num.trees = 5, 
  ...
  dt_cont <- data.frame(x = sample(c("A"), n, replace = TRUE), 
                        y = rnorm(n),
                        stringsAsFactors = FALSE)
  expect_silent(crazyforest(y ~ ., data = dt_cont, num.trees = 5, 
  ...
  dt_surv <- data.frame(x = sample(c("A"), n, replace = TRUE), 
                        ...
                        status = rbinom(n, 1, .5),
                        stringsAsFactors = FALSE)
  expect_silent(crazyforest(Surv(time, status) ~ ., data = dt_surv, num.trees = 5, 
                       respect.unordered.factors = 'order'))
})

test_that("Unordered splitting working for survival", {
  rf <- crazyforest(Surv(time, status) ~ ., veteran, num.trees = 5, min.node.size = 50, respect.unordered.factors = 'partition')
  expect_true(any(!rf$forest$is.ordered))
})

test_that("Order splitting working for survival", {
  rf <- crazyforest(Surv(time, status) ~ ., veteran, num.trees = 5, min.node.size = 50, respect.unordered.factors = 'order')
  expect_true(all(rf$forest$is.ordered))
})

test_that("Order splitting working for survival with alternative interface", {
  rf <- crazyforest(dependent.variable.name = "time", status.variable.name = "status", data = veteran, num.trees = 5, min.node.size = 50, respect.unordered.factors = 'order')
  expect_true(all(rf$forest$is.ordered))
})
...
test_that("Error if too many factors in 'partition' mode", {
  ...
  expect_error(crazyforest(y ~ ., data = dt, num.trees = 5, respect.unordered.factors = 'partition'))
})
...
test_that("Survival forest with 'order' mode works", {
  ...
  expect_is(pred, "crazyforest.prediction")
})

test_that("maxstat splitting not working with unordered factors", {
  expect_error(crazyforest(Sepal.Length ~ ., iris, splitrule = "maxstat", respect.unordered.factors = "partition"))
  expect_error(crazyforest(Surv(time, status) ~ ., veteran, splitrule = "maxstat", respect.unordered.factors = "partition"))
})

test_that("C splitting not working with unordered factors", {
  expect_error(crazyforest(Surv(time, status) ~ ., veteran, splitrule = "C", respect.unordered.factors = "partition"))
})

test_that("Warning for maxstat with 'order' mode", {
  expect_warning(crazyforest(Sepal.Length ~ ., iris, num.trees = 5, splitrule = "maxstat",
                        respect.unordered.factors = 'order'))
})

test_that("No warning for multiclass classification/probability and survival with 'order' mode", {
  expect_silent(crazyforest(Species ~ ., iris, num.trees = 5, 
                        respect.unordered.factors = 'order'))
  expect_silent(crazyforest(Species ~ ., iris, num.trees = 5, probability = TRUE,
                        respect.unordered.factors = 'order'))
  expect_silent(crazyforest(Surv(time, status) ~ ., veteran, num.trees = 5, 
                        respect.unordered.factors = 'order'))
})
...
test_that("No error if new levels in predict, 1 column", {
  ...
  ## ignore
  rf.ignore <- crazyforest(y ~ ., data = train, num.trees = 5, respect.unordered.factors = 'ignore')
  expect_silent(predict(rf.ignore, test))

  ## partition  
  rf.partition <- crazyforest(y ~ ., data = train, num.trees = 5, respect.unordered.factors = 'partition')
  expect_silent(predict(rf.partition, test))
  
  ## order
  rf.order <- crazyforest(y ~ ., data = train, num.trees = 5, respect.unordered.factors = 'order')
...
test_that("No error if NA factor levels and order", {
  df <- data.frame(x = addNA(factor(c("a", "a", NA, NA, "b", "b"))),
                   y = c(1, 2, 3, 4, 5, 6))
  expect_silent(crazyforest(dependent.variable.name = "y", data = df, 
                respect.unordered.factors = "order"))
})
...
test_that("Order splitting working when numerics in data", {
  ...
  dt_class <- data.frame(x1 = sample(c("A", "B", "C"), n, replace = TRUE), 
                         ...
                         y = factor(sample(c("A", "B"), n, replace = TRUE)),
                         stringsAsFactors = FALSE)
  rf <- expect_silent(crazyforest(y ~ ., data = dt_class, num.trees = 5, 
                       respect.unordered.factors = 'order', probability = FALSE))
  ...
  dt_cont <- data.frame(x1 = sample(c("A", "B", "C"), n, replace = TRUE), 
                        ...
                        y = rnorm(n),
                        stringsAsFactors = FALSE)
  rf <- expect_silent(crazyforest(y ~ ., data = dt_cont, num.trees = 5, 
                       respect.unordered.factors = 'order'))
  ...
  dt_surv <- data.frame(x1 = sample(c("A", "B", "C"), n, replace = TRUE), 
                        ...
                        status = rbinom(n, 1, .5),
                        stringsAsFactors = FALSE)
  rf <- expect_silent(crazyforest(Surv(time, status) ~ ., data = dt_surv, num.trees = 5, 
                       respect.unordered.factors = 'order'))
  ...
  dt_surv <- data.frame(x1 = sample(c("A", "B", "C"), n, replace = TRUE), 
                        ...
                        stringsAsFactors = FALSE)
  rf <- expect_silent(crazyforest(y ~ ., data = dt_surv, num.trees = 5, 
                             respect.unordered.factors = 'order'))
  expect_silent(predict(rf, dt_surv))
})
...
test_that("Order splitting working for quantreg forests", {
  ...
  expect_silent(crazyforest(y ~ ., dt, num.trees = 5,
