...
library(crazyforest)
context("crazyforest_imp")
set.seed(123)

## Classification
rg.imp.class <- crazyforest(Species ~ ., data = iris,
                       num.trees = 5, importance = "impurity")
rg.perm.class <- crazyforest(Species ~ ., data = iris,
                        num.trees = 5, importance = "permutation")
rg.scale.perm.class <- crazyforest(Species ~ ., data = iris, 
                              num.trees = 5, importance = "permutation", scale.permutation.importance = TRUE)

## Probability estimation
rg.imp.prob <- crazyforest(Species ~ ., data = iris, 
                      num.trees = 5, importance = "impurity", probability = TRUE)
rg.perm.prob <- crazyforest(Species ~ ., data = iris, 
                      num.trees = 5, importance = "permutation", probability = TRUE)
rg.scale.perm.prob <- crazyforest(Species ~ ., data = iris, 
                             num.trees = 5, importance = "permutation", scale.permutation.importance = TRUE, probability = TRUE)

## Regression
rg.imp.regr <- crazyforest(Sepal.Length ~ ., data = iris, 
                      num.trees = 5, importance = "impurity")
rg.perm.regr <- crazyforest(Sepal.Length ~ ., data = iris, 
                       num.trees = 5, importance = "permutation")
rg.scale.perm.regr <- crazyforest(Sepal.Length ~ ., data = iris, 
                             num.trees = 5, importance = "permutation", scale.permutation.importance = TRUE)

## Survival
rg.perm.surv <- crazyforest(Surv(time, status) ~ ., data = veteran, 
                       num.trees = 5, importance = "permutation")
rg.scale.perm.surv <- crazyforest(Surv(time, status) ~ ., data = veteran, 
                             num.trees = 5, importance = "permutation", scale.permutation.importance = TRUE)
