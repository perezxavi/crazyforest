...
test_that("Alternative interface works for regression", {
  rf <- crazyforest(dependent.variable.name = "Sepal.Length", data = iris)
  expect_equal(rf$treetype, "Regression")
})

test_that("Matrix interface works for regression", {
  rf <- crazyforest(dependent.variable.name = "Sepal.Length", data = data.matrix(iris), write.forest = TRUE)
  expect_equal(rf$treetype, "Regression")
...
test_that("Matrix interface prediction works for regression", {
  ...
  rf <- crazyforest(dependent.variable.name = "Sepal.Length", data = dat, write.forest = TRUE)
  expect_silent(predict(rf, dat))
})

test_that("save.memory option works for regression", {
  rf <- crazyforest(Sepal.Length ~ ., data = iris, save.memory = TRUE)
  expect_equal(rf$treetype, "Regression")
})
...
test_that("Variance splitting not working on classification data", {
  expect_error(crazyforest(Species ~ ., iris, splitrule = "variance"))
})
