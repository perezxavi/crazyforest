...
library(crazyforest)
context("crazyforest_classweights")

test_that("No error if class weights used", {
  expect_silent(crazyforest(Species ~ ., iris, num.trees = 5, class.weights = c(0.5, 1, 0.1)))
})
...
test_that("Prediction accuracy for minority class increases with higher weight", {
  ...
  rf <- crazyforest(y ~ ., dat, num.trees = 50, min.node.size = 50, class.weights = c(1, 1))
  acc_major <- mean((rf$predictions == dat$y)[dat$y == "0"], na.rm = TRUE)
  acc_minor <- mean((rf$predictions == dat$y)[dat$y == "1"], na.rm = TRUE)

  rf <- crazyforest(y ~ ., dat, num.trees = 5, min.node.size = 50, class.weights = c(0.01, 0.99))
...
#   rf_weighted <- crazyforest(Species ~ ., iris, num.trees = 5, class.weights = c(1, 0, 0))
#   expect_lt(rf$prediction.error, rf_weighted$prediction.error)
# })

test_that("Error if class weights of wrong size", {
  expect_error(crazyforest(Species ~ ., iris, num.trees = 5, class.weights = c(0.5, 1)),
               "Error: Number of class weights not equal to number of classes.")
})

test_that("Error if class weights NA", {
  expect_error(crazyforest(Species ~ ., iris, num.trees = 5, class.weights = c(0.5, 1, NA)),
               "missing value where TRUE/FALSE needed")
})

test_that("Error if class weights not numeric", {
  expect_error(crazyforest(Species ~ ., iris, num.trees = 5, class.weights = c(0.5, 1, "a")),
