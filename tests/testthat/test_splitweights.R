...
library(crazyforest)
context("crazyforest_splitweights")

## Tests
test_that("split select weights work", {
  expect_silent(crazyforest(Species ~ ., iris, num.trees = 5, split.select.weights = c(0.1, 0.2, 0.3, 0.4)))
  expect_error(crazyforest(Species ~ ., iris, num.trees = 5, split.select.weights = c(0.1, 0.2, 0.3)))
})

test_that("split select weights work with 0s and 1s", {
  num.trees <- 5
  weights <- replicate(num.trees, sample(c(0, 0, 1, 1)), simplify = FALSE)
  rf <- crazyforest(Species ~ ., iris, num.trees = num.trees, split.select.weights = weights)
...
test_that("Tree-wise split select weights work", {
  ...
  expect_silent(crazyforest(Species ~ ., iris, num.trees = num.trees, split.select.weights = weights))
  
  weights <- replicate(num.trees+1, runif(ncol(iris)-1), simplify = FALSE)
  expect_error(crazyforest(Species ~ ., iris, num.trees = num.trees, split.select.weights = weights))
})

test_that("always split variables work", {
  expect_silent(crazyforest(Species ~ ., iris, num.trees = 10, 
                       always.split.variables = c("Petal.Length", "Petal.Width"), mtry = 2))
  expect_silent(crazyforest(Species ~ ., iris, num.trees = 10, 
                       always.split.variables = c("Petal.Width", "Petal.Length"), mtry = 2))
  expect_silent(crazyforest(dependent.variable.name = "Species", data = iris, num.trees = 10, 
                       always.split.variables = c("Petal.Length", "Petal.Width"), mtry = 2))
})
...
test_that("Tree-wise split select weights work with 0s", {
  ...
  rf <- crazyforest(Species ~ ., iris, mtry = 2, num.trees = num.trees, 
...
test_that("always split variables respect split select weights", {
    ...
    expect_silent(
        crazyforest(Species ~ ., iris, num.trees=5,
