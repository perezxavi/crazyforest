...
Rcpp::List crazyforestCpp(
    ...
    Rcpp::NumericMatrix &input_y, std::vector<std::string> variable_names,
    unsigned int mtry, unsigned int num_trees, bool verbose, unsigned int seed,
    unsigned int num_threads, bool write_forest, unsigned int importance_mode_r,
    std::vector<unsigned int> &min_node_size,
    std::vector<unsigned int> &min_bucket,
    std::vector<std::vector<double>> &split_select_weights,
    bool use_split_select_weights,
    std::vector<std::string> &always_split_variable_names,
    bool use_always_split_variable_names, bool prediction_mode,
    Rcpp::List loaded_forest, Rcpp::RawMatrix snp_data,
    ...
    unsigned int splitrule_r, std::vector<double> &case_weights,
    bool use_case_weights, std::vector<double> &class_weights, bool predict_all,
    bool keep_inbag, std::vector<double> &sample_fraction, double alpha,
    double minprop, double poisson_tau, bool holdout,
    unsigned int prediction_type_r, unsigned int num_random_splits,
    Eigen::SparseMatrix<double> &sparse_x, bool use_sparse_data,
    bool order_snps, bool oob_error, unsigned int max_depth,
    std::vector<std::vector<size_t>> &inbag, bool use_inbag,
    std::vector<double> &regularization_factor, bool use_regularization_factor,
    bool regularization_usedepth, bool node_stats,
    std::vector<double> &time_interest, bool use_time_interest, bool any_na,
    unsigned int predict_strategy_r, Rcpp::NumericVector class_frequencies_r) {
  ...
  try {
    ...
    // Initialize data
    if (use_sparse_data) {
      data = std::make_unique<DataSparse>(sparse_x, input_y, variable_names,
                                          num_rows, num_cols, any_na);
    } else {
      data = std::make_unique<DataRcpp>(input_x, input_y, variable_names,
                                        num_rows, num_cols, any_na);
    }
    ...
    PredictStrategy predict_strategy = (PredictStrategy)predict_strategy_r;

    // Init CrazyForest
    forest->initR(
        std::move(data), mtry, num_trees, verbose_out, seed, num_threads,
        importance_mode, min_node_size, min_bucket, split_select_weights,
        always_split_variable_names, prediction_mode, sample_with_replacement,
        unordered_variable_names, save_memory, splitrule, case_weights, inbag,
        predict_all, keep_inbag, sample_fraction, alpha, minprop, poisson_tau,
    ...
    std::vector<double> class_frequencies =
        Rcpp::as<std::vector<double>>(class_frequencies_r);
    forest->setPredictStrategy(predict_strategy);
    forest->setClassFrequencies(class_frequencies);

    // Load forest object if in prediction mode
    if (prediction_mode) {
      std::vector<std::vector<std::vector<size_t>>> child_nodeIDs =
          loaded_forest["child.nodeIDs"];
      std::vector<std::vector<size_t>> split_varIDs =
          loaded_forest["split.varIDs"];
      std::vector<std::vector<double>> split_values =
          loaded_forest["split.values"];
      ...
      if (treetype == TREE_CLASSIFICATION) {
        ...
        temp.loadForest(num_trees, child_nodeIDs, split_varIDs, split_values,
                        class_values, is_ordered);
        ...
        std::vector<double> unique_timepoints =
            loaded_forest["unique.death.times"];
        auto &temp = dynamic_cast<ForestSurvival &>(*forest);
        temp.loadForest(num_trees, child_nodeIDs, split_varIDs, split_values,
                        chf, unique_timepoints, is_ordered);
    ...
    result.push_back(forest->getNumIndependentVariables(),
    ...
    if (!prediction_mode) {
      ...
      if (importance_mode != IMP_NONE) {
        result.push_back(forest->getVariableImportance(),
                         "variable.importance");
        if (importance_mode == IMP_PERM_CASEWISE) {
          result.push_back(forest->getVariableImportanceCasewise(),
    ...
    if (write_forest) {
      ...
      if (node_stats) {
        forest_object.push_back(forest->getNumSamplesNodes(),
                                "num.samples.nodes");
      ...
      if (snp_data.nrow() > 1 && order_snps) {
        ...
        forest_object.push_back(
            std::vector<std::vector<size_t>>(
                snp_order.begin(), snp_order.begin() + snp_data.ncol()),
            "snp.order");
      }
      ...
      if (treetype == TREE_CLASSIFICATION) {
        ...
        if (node_stats) {
          forest_object.push_back(forest->getNodePredictions(),
                                  "node.predictions");
        }
      } else if (treetype == TREE_REGRESSION) {
        if (node_stats) {
        ...
        auto &temp = dynamic_cast<ForestProbability &>(*forest);
        forest_object.push_back(temp.getClassValues(), "class.values");
        forest_object.push_back(temp.getTerminalClassCounts(),
                                "terminal.class.counts");
      } else if (treetype == TREE_SURVIVAL) {
        auto &temp = dynamic_cast<ForestSurvival &>(*forest);
        forest_object.push_back(temp.getChf(), "chf");
        forest_object.push_back(temp.getUniqueTimepoints(),
                                "unique.death.times");
      }
    ...
    if (strcmp(e.what(), "User interrupt.") != 0) {
      Rcpp::Rcerr << "Error: " << e.what() << " CrazyForest will EXIT now."
                  << std::endl;
    }
    return result;
  }
