...
ForestProbability::getTerminalClassCounts() const {
...
void ForestProbability::allocatePredictMemory() {
  ...
  if (predict_all) {
    predictions = std::vector<std::vector<std::vector<double>>>(
        ...
        std::vector<std::vector<double>>(class_values.size(),
    ...
    predictions = std::vector<std::vector<std::vector<double>>>(
        1, std::vector<std::vector<double>>(
               num_prediction_samples,
               std::vector<double>(class_values.size(), 0)));
  }
}
...
void ForestProbability::predictInternal(size_t sample_idx) {
  ...
  if (!predict_all && prediction_type != TERMINALNODES) {
    for (size_t class_idx = 0; class_idx < predictions[0][sample_idx].size();
         ++class_idx) {
...
void ForestProbability::computePredictionErrorInternal() {
  ...
  predictions = std::vector<std::vector<std::vector<double>>>(
      1, std::vector<std::vector<double>>(
             num_samples, std::vector<double>(class_values.size(), 0)));
...
void ForestProbability::writeOutputInternal() {
  if (verbose_out) {
    *verbose_out << "Tree type:                         "
                 << "Probability estimation" << std::endl;
  }
}
...
void ForestProbability::loadFromFileInternal(std::ifstream &infile) {
  ...
  if (treetype != TREE_PROBABILITY) {
    throw std::runtime_error(
        "Wrong treetype. Loaded file is not a probability estimation forest.");
  }
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    ...
    // Convert Terminal node class counts to vector with empty elemtents for
    // non-terminal nodes
    std::vector<std::vector<double>> terminal_class_counts;
    terminal_class_counts.resize(child_nodeIDs[0].size(),
                                 std::vector<double>());
    for (size_t j = 0; j < terminal_nodes.size(); ++j) {
      terminal_class_counts[terminal_nodes[j]] =
          terminal_class_counts_vector[j];
    }
    ...
    trees.push_back(std::make_unique<TreeProbability>(
        ...
        &response_classIDs, terminal_class_counts));
  }
}

const std::vector<double> &
ForestProbability::getTreePrediction(size_t tree_idx, size_t sample_idx) const {
  const auto &tree = dynamic_cast<const TreeProbability &>(*trees[tree_idx]);
  return tree.getPrediction(sample_idx);
}

size_t
ForestProbability::getTreePredictionTerminalNodeID(size_t tree_idx,
